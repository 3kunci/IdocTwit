...
//  CGPoint viewOrigin   = [(UIView *)[self.window.subviews objectAtIndex:0] frame].origin;
    CGPoint viewOrigin = CGPointMake(0.0, 20.0);
  ...
  [self.nextViewLayer addAnimation:nextViewAnimation forKey:@"NextViewAnimationStarted"];
    ...
    [[self.sourceViewController view] setHidden:YES];
...
- (void)animationDidStop:(CAAnimation *)animation finished:(BOOL)isFinished
{
  if (isFinished)
  {
      ...
      UIViewController *container = [self.window rootViewController];
      [container addChildViewController:dst];
      [self.window addSubview:dst.view];
      [dst didMoveToParentViewController:container];
      
      [self.doorLayerLeft removeFromSuperlayer];
      [self.doorLayerRight removeFromSuperlayer];
      [self.nextViewLayer removeFromSuperlayer];
      
      [[self.sourceViewController view] setHidden:NO];
  }
}
